...
class Boletos {
    ...
    class func getValuesBoletoLidos(CodigoBarras:String)->NSDictionary{
        ...
        let TamanhoCodigo = CodigoBarras.count
        ...
        if TamanhoCodigo >= 44 {
            ...
            if (CodigoBarras as NSString).substring(with: NSRange(location: 0, length: 1)) == "8" {
                
                //SEGMENTO - TIPO CONVENIO POSICAO 2 - 2
                let id_segmento = (CodigoBarras as NSString).substring(with: NSRange(location: 1, length: 1))
                
                //VALOR POSICAO
                let id_valor = (CodigoBarras as NSString).substring(with: NSRange(location: 4, length: 11))
                
                //VENCIMENTO
                //let vencimento = (CodigoBarras as NSString).substringWithRange(NSRange(location: 5, length: 4))
                
                //PEGA NOME DO CONVENIO
                NomeBanco = getNameConvenios(CodigoSegmento: id_segmento)
...
//
//                //VALOR POSICAO
//                let id_valor = (CodigoBarras as NSString).substring(with: NSRange(location: 37, length: CodigoBarras.count - 37))
//
//                //VENCIMENTO
//                //let vencimento = (CodigoBarras as NSString).substringWithRange(NSRange(location: 5, length: 4))
//
...
//                Valor = id_valor
                ...
                let id_banco = (CodigoBarras as NSString).substring(with: NSRange(location: 0, length: 3))
                
                //Fator de vencimento (posições 6 a 9 do código de barras)
                let fator_vencimento = (CodigoBarras as NSString).substring(with: NSRange(location: 33, length: 4))
                
                // Valor nominal do título (posições 10 a 19 do código de barras)
                let valor_nominal = (CodigoBarras as NSString).substring(with: NSRange(location: 37, length: CodigoBarras.count - 37))
                
                
                NomeBanco = getNameBancos(Codigo: id_banco)
                Valor = valor_nominal
                Vencimento = getVencimento(fatorVencimento: fator_vencimento)
            }
        }
    ...
    class func getValuesBoletosDigitatos(CodigoBarras:String)->NSDictionary{
        ...
        if TamanhoCodigo >= 44 {
            ...
            if (CodigoBarras as NSString).substring(with: NSRange(location: 0, length: 1)) == "8"{
                ...
                //Fator de vencimento (posições 6 a 9 do código de barras)
                let fator_vencimento = (CodigoBarras as NSString).substring(with: NSRange(location: 33, length: 4))
                //let fator_vencimento = (CodigoBarras as NSString).substring(with: NSRange(location: 10, length: 1))
                
                // Valor nominal do título (posições 10 a 19 do código de barras)
                let valor_nominal = (CodigoBarras as NSString).substring(with: NSRange(location: 37, length: 10))
