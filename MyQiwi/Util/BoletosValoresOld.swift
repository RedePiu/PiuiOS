    ...
    class func getValuesBoletoLidos(CodigoBarras:String)->NSDictionary{
        ...
        if TamanhoCodigo >= 44 {
            ...
            if (CodigoBarras as NSString).substring(with: NSRange(location: 0, length: 1)) == "8" {
                ...
                let fator_vencimento = (CodigoBarras as NSString).substring(with: NSRange(location: 5, length: 4))
                
                // Valor nominal do título (posições 10 a 19 do código de barras)
                let valor_nominal = (CodigoBarras as NSString).substring(with: NSRange(location: 9, length: 10))
